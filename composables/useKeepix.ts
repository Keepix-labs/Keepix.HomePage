...
export const useKeepix = () => {
  ...
  const getAll = async () => {
    ...
    for (let i = 1; i < 255; i++) {
      const fetchPromise = fetch(
        `https://${subdomains[i - 1]}.keepix.org:9000/app`,
        {
  ...
  const getAllSetupKeepix = () => {
    ...
    if (item) {
      return JSON.parse(item) as Keepix[];
    }

    return [];
  };

  const setSetupKeepix = (keepix: Keepix) => {
    const item = window.localStorage.getItem("keepix-setup");
    let value = [keepix];
    ...
    if (item) {
      ...
      if (isSetupKeepix(keepix)) {
        const index = value.findIndex(
          (storedKeepix) => storedKeepix.ip === keepix.ip
        );
  ...
  const isSetupKeepix = (keepix: Keepix) => {
    const list = getAllSetupKeepix();

    if (list) {
      return list.some((storedKeepix) => storedKeepix.ip === keepix.ip);
    }
